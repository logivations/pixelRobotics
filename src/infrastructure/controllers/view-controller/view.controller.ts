...
export class ViewController {
  ...
  newsAndEventsDetails(@Res() res, @Req() request: FastifyRequest) {
    ...
    return res.view(getViewNameByLang(request, 'news.and.events.details.ejs'), {
      event,
    });
  }
  ...
  career(@Res() res, @Req() request: FastifyRequest) {
    ...
    const vacancies =
      careers[lang && lang === 'DE_de' ? 'vacanciesDE' : 'vacanciesEN'];
    return res.view(getViewNameByLang(request, 'careers.ejs'), { vacancies });
  }
  ...
  registration(@Res() res, @Req() request: FastifyRequest) {
    return res.view(getViewNameByLang(request, 'registration.ejs'));
  }
