...
export class AdminController {
  ...
  async getUserActivity(
    ...
    const paginateItems = await this.userActivityService.paginate({
      limit: 50,
      page: page,
    });
    const data = this.adminService.modifyData(
      paginateItems.items,
      paginateItems.meta,
    );
  ...
  checkPassword(
    ...
    if (
      ...
      this.adminService.checkPassword(password)
    ) {
