...
export class UserActivityService {
  ...
  public async setResolution(request: Request, resolution: Resolution): Promise<void> {
    ...
    await this.userActivityEntityRepository
      ...
      .andWhere({ userIP: ip, providerDetail, userAgent: request.headers['user-agent'] })
      .execute();
  }
  ...
  private async getProviderDetail(userIP: string) {
    const providerData = await this.httpService.get(`http://ip-api.com/json/${userIP}`).toPromise();
    // @ts-ignore
